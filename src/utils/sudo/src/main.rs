...
fn main() {
    ...
    let args = App::new("sudo")
                ...
                .arg(Arg::with_name("user").short("u").long("user").takes_value(true).help("Run the command as a user other than root"))
                .arg(Arg::with_name("group").short("g").long("group").takes_value(true).help("Run the command with the primary group as something other than root"))
                .arg(Arg::with_name("command").takes_value(true).multiple(true).help("The command to run"))
                .get_matches();
