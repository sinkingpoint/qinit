...
use libq::passwd::{PasswdEntry, GroupEntry, Groups};
...
fn main() {
    let args = App::new("id")
                ...
                .arg(Arg::with_name("user").short("u").help("Only print the user ID").conflicts_with_all(&["group", "groups"]))
                .arg(Arg::with_name("group").short("g").help("Only print the group ID").conflicts_with_all(&["user", "groups"]))
                .arg(Arg::with_name("groups").short("G").help("Print all group IDs").conflicts_with_all(&["group", "user"]))
    ...
    if group {
        ...
        if name {
            ...
            println!("{} {}", gid.gid, groups.map(|group| group.gid.to_string()).collect::<Vec<String>>().join(" "));
        }
    }
    else {
