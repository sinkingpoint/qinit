...
use patient::{FreudianClient, FreudianClientError, ResponseType};
...
fn friendly_error<'a>(f: FreudianClientError) -> String {
    return match f {
        ...
        FreudianClientError::ResponseTooSmall => {
            String::from("Got a response, but the advertised message size was more than the data sent")
        }
...
fn handle_response_message(
    response: Result<(ResponseType, String), FreudianClientError>,
    body_preamble: &str,
    logger: &Logger<ConsoleRecordWriter>,
) {
    match response {
        ...
        ("topic", Some(matches)) => match matches.subcommand() {
            ...
            ("publish", Some(matches)) => {
                handle_status_message(
                    client.send_message(matches.value_of("topic_name").unwrap(), matches.value_of("message").unwrap()),
                    &logger,
                );
            }
            _ => {}
        ...
        ("subscription", Some(matches)) => match matches.subcommand() {
            ("create", Some(matches)) => {
                handle_response_message(
                    client.create_subscription(matches.value_of("topic_name").unwrap()),
                    "Subscription ID: ",
                    &logger,
                );
            }
