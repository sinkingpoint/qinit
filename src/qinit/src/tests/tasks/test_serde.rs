...
use accountant::tasks::serde::{DependencyDef, Identifier, TaskDef};
...
fn test_dependency_def_eq() {
    assert_eq!(
        DependencyDef::new("test".to_owned(), None),
        DependencyDef::new("TEST".to_owned(), None)
    );
    assert_ne!(
        DependencyDef::new("test".to_owned(), None),
        DependencyDef::new("TES".to_owned(), None)
    );
    assert_eq!(
        DependencyDef::new(
            "test".to_owned(),
            Some(vec![("arg".to_owned(), "arg".to_owned())].into_iter().collect())
        ),
        DependencyDef::new(
            "test".to_owned(),
            Some(vec![("arg".to_owned(), "arg".to_owned())].into_iter().collect())
    ...
    assert_ne!(
        ...
        DependencyDef::new(
            "test".to_owned(),
            Some(vec![("arg".to_owned(), "arg1".to_owned())].into_iter().collect())
        )
    );
    assert_ne!(
        DependencyDef::new(
    ...
    assert_ne!(
        ...
        DependencyDef::new(
            "test".to_owned(),
            Some(vec![("gra".to_owned(), "arg".to_owned())].into_iter().collect())
        )
    );
}
...
fn test_task_deserializes() {
    let test = toml::from_str::<TaskDef>(
        "name = \"TestTask\"
    ...
    assert_eq!(
        test.requires.as_ref().unwrap()[0].args.as_ref().unwrap().get("test_arg").unwrap(),
        "test"
    );
    ...
    assert_eq!(
        test.requires.as_ref().unwrap()[1].args.as_ref().unwrap().get("test_arg2").unwrap(),
        "test2"
    );
    assert_eq!(
        test.requires.as_ref().unwrap()[1].args.as_ref().unwrap().get("test_arg3").unwrap(),
        "test3"
    );

    assert_eq!(test.conditions.as_ref().unwrap().unix_sockets.as_ref().unwrap().len(), 1);
    assert_eq!(
        test.conditions.as_ref().unwrap().unix_sockets.as_ref().unwrap()[0].path,
        Path::new("/var/run/socket")
    );
}
