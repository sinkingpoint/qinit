...
use accountant::tasks::{listen_for_children, CourthouseBuilder, DependencyDef, SphereRegistry};
use std::path::Path;
use std::sync::{Arc, Mutex};

fn main() {
    let registry = SphereRegistry::read_from_disk(vec![
        &Path::new("/etc/qinit/tasks"),
        &Path::new("src/rootfs/tasks"),
        &Path::new("/cats"),
    ])
    .unwrap();
